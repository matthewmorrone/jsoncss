...
var Absurd = (function(w) {
	...
	var Component = function(componentName, absurd, eventBus, cls) {
		...
		api.onAnimationEnd = function(el, func) {
			...
			if (!eventName) {
				func.apply(this, [{
					error: 'Animations not supported.'
				}]);
				return;
			};
	...
	var client = function() {
		return function(arg) {
			...
			_api.compile = function(callback, options) {
					...
					var res = options.processor(_api.getRules(), callback || function() {}, options);
					_api.flush();
					return res;
				}
	...
	lib.api.add = function(API) {
		...
		var addRule = function(selector, props, stylesheet, parentSelector) {
			...
			for (var prop in _objects) {
				...
				if (prop.charAt(0) === ":") {
					...
					addRule(selector.substr(1, selector.length - 1) + (typeof parentSelector !== "undefined" ? " " + parentSelector : '') + " " + prop, _objects[prop], stylesheet);
					// check for plugins
		...
		var add = function(rules, stylesheet, opts) {
			...
			try {
				...
				throw new Error("Error adding: " + JSON.stringify({
					rules: rules,
					error: err.toString()
				}));
			}
	...
	lib.helpers.CSSParse = function(css, options) {
		...
		function atrule() {
			if (css[0] != '@') return;
			return atkeyframes() || atmedia() || atsupports() || atimport() || atcharset() || atnamespace() || atdocument() || atpage() || athost() || atfontface();
		}
		/**
		 * Parse rule.
		 */
	...
	lib.helpers.Prefixes = {
		addPrefixes: function(prop, obj) {
			...
			if (p.prefix !== false) {
				...
				obj[p.prop] = value;
				if (p.prefix === '' || p.prefix.indexOf('w') >= 0) obj['-webkit-' + p.prop] = value;
				if (p.prefix === '' || p.prefix.indexOf('m') >= 0) obj['-moz-' + p.prop] = value;
				if (p.prefix === '' || p.prefix.indexOf('s') >= 0) obj['-ms-' + p.prop] = value;
				if (p.prefix === '' || p.prefix.indexOf('o') >= 0) obj['-o-' + p.prop] = value;
			}
		},
	...
	var prepareHTML = function(html) {
		...
		if (passedOptions.minify) {
			...
			return beautifyHTML(html, {
				indent_size: passedOptions.indentSize || 4
			});
		}
	}
